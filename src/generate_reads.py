...
def generate_reads(sequence, num_reads, read_length):
    ...
    if genome_len < read_length:
        print(f"Error: Genome length ({genome_len}) is smaller than read length ({read_length})")
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description="Generate simulated reads from a reference genome.")
    parser.add_argument("--fasta", required=True, help="Path to input reference FASTA file")
    parser.add_argument("--num_reads", type=int, required=True, help="Number of reads to generate")
    parser.add_argument("--read_len", type=int, default=100, help="Length of each read (default: 100)")
    parser.add_argument("--output", required=True, help="Path to output FASTA file for generated reads")
